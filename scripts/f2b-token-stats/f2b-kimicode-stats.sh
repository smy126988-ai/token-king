#!/bin/bash
# f2b-kimicode-stats.sh — canonical KimiCode token usage report.
#
# Reads the local F2b SQLite at $F2B_DB_PATH (default: macOS app support)
# and prints a consistent five-section report for rows with
# `source='kimiCode'` (MoonshotAI/kimi-code 0.23+ wire.jsonl sessions):
#
...
#
# Schema notes (verified against ~/kimi-code/sessions/.../agents/main/wire.jsonl
# on kimi-code 0.23.1):
#   - Line shape:
...
#        "time":<epoch-ms>}
#   - Usage object is one level deeper than top-level; falls back through
#     KimiCodeExtractor.parseFile (top-level `usage`, then `event.usage`).
#   - `inputCacheRead` is the per-step cache-hit count (Anthropic-style
#     semantics). Sum across events = total cache hits billed. Matches
#     Codex/OpenCode per-request contract; raw sum is the correct algorithm.
#
# Token columns match f2b-opencode-stats.sh / f2b-codex-stats.sh so rows
# can be diffed directly across sources:
#   input/output/reasoning — billed at full rate
#   cache_read  — billed at a discounted rate (NOT free)
#   cache_write — billed at higher rate on Anthropic-style APIs (typically 0 here)
#   billable    — input + output + reasoning (strict subscription cost)
#
# Usage:
#   ./f2b-kimicode-stats.sh
#   ./f2b-kimicode-stats.sh --since 2026-07
#   ./f2b-kimicode-stats.sh --provider kimi
#   ./f2b-kimicode-stats.sh --top-models 30
#   ./f2b-kimicode-stats.sh --db /path/to/other.sqlite
set -euo pipefail
...
print_header() {
    ...
    echo ""
    echo "=================================================================="
    echo "  F2b KimiCode stats — $section"
