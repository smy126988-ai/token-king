...
final class StatusBarControllerRedesignTests: XCTestCase {
    ...
    func testPerProviderTokenSubmenuRendersInputOutputCacheTotal() {
        ...
        XCTAssertTrue(texts.contains(where: { $0.contains("Total:") && $0.contains("400") }), "Claude Total:200+100+60+40=400 row missing in \(texts)")
    }
    ...
    func testPerProviderTokenSubmenuIncludesReasoningInTotal() {
        let totals = [
            PerPeriodTokenAggregator.ProviderTotal(
                providerRaw: "kimi", displayName: "Kimi",
        ...
        let texts = extractText(from: menu)
        // Total = 100+50+30+20+7 = 207
        XCTAssertTrue(texts.contains(where: { $0.contains("Total:") && $0.contains("207") }), "Total should include reasoning; got \(texts)")
    }
