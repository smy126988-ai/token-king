...
# 追加 2:Round 7 — 方向纠错,小尺寸定案为"迷你 QuotaCard"(用户验收通过)

## 关键纠错

此前 6 轮都在复刻 quota-float 的**收缩态 QuotaOrb**(纯数字球),用户连续否决。
用户最终给出参照图 `quota-states.png`(quota-float 仓库 docs/images/):他要的是
**展开态 QuotaCard** 的信息层级。**教训:先拿到竞品的真实渲染图再动手,CSS 源码
不足以锚定"渲染出来的样子";且要先确认用户心中的参照物是产品的哪一种形态。**

## 定案设计(small)

- 层级:eyebrow(名字大写宽字距 9px)+ 右上发光状态点 → "5-hour remaining"(8px)
  → 45px 剩余%大数字 + 17px bold % → 渐变发光条(宽=剩余、色=已用 tier)→
  "resets in Xh Ym"(8px/52%)。
- 容器(containerBackground):实色浅卡 orbCardBackground + 同 tier aurora(短窗 tier)
  + 1px 白描边(.34→顶 .42)。vibrant/accented 由系统接管,内容无需门控。
- 数据:shortWindow(5h)剩余 %;usage provider 回退 $ 金额。
- token(只加):miniEyebrowSize 9 / miniEyebrowTracking 1.7 / miniDescriptorSize 8 /
  miniResetSize 8 / miniBarTopMargin 9;Aurora.Tier 增补 warmX/warmY(quota-float
  --warm-position,各 tier 不同);AuroraBackgroundView 增补 init(tier:)。

## 证据

编译 0 error;swiftlint 0;裸值 grep 空;中文 grep 空;#Preview 10。
用户真机目检:"这次不错了"。

## 方法论沉淀(这次做对了什么)

1. **先锁渲染参照物,再写代码**:CSS 精确值 ≠ 渲染效果。两轮"按源码照抄"都偏,
   直到下载 quota-orb.png / quota-states.png 才锚定。
2. **先确认用户要的是产品的哪种形态**(orb 球 vs 展开卡),再谈复刻精度。
3. **抄结构与数据语义,不迷信像素**:层级、剩余%语义、tier 配色、条宽=剩余,这些
   照抄;像素值按画布等比换算,可读性下限(8-9px)优先。
4. **背景分层交给系统**:浅卡+aurora 进 containerBackground,vibrant 零门控代码。
