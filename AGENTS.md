...
### F1 / F3 / F4 Implementation Patterns (2026-07-08 / 2026-07-09)
- **Pure Function Aggregators for Testability**: `TokenStatsAggregator` is a pure function over a list of `DayAggregate` (static `snapshot(dayAggregates:todayString:weekStart:weekEnd:monthPrefix:)`). All async SQLite I/O lives in the caller (`StatusBarController.computeTokenStatsSnapshot`). This split makes the aggregator trivially testable without actor / UserDefaults setup. Pattern: `enum XAggregator { struct Snapshot { ... }; static func snapshot(inputs: [...]) -> Snapshot }`.
- **Testable View Builders via Precomputed Inputs**: `StatusBarController.createGlobalStatsSubmenu(snapshot:)` takes a precomputed `TokenStatsAggregator.Snapshot` instead of a `TokenUsageStore` actor. Tests pass a hand-built snapshot (e.g. `Snapshot(todayTotal: TokenBreakdown(input: 1500), …)`); production invokes it from a private async helper. Pattern: separate "build view from data" from "fetch data" — enables synchronous, fast, deterministic UI tests with no `XCTestExpectation` dance.
- **Group Aggregates Before Iterating for UI**: First F1 pass iterated `dayAggregates` directly, producing per-day-per-model rows. With 2 models × 2 days that's 4 rows in both the "monthly" and "daily" sections and the `NSUserInterfaceItemIdentifier`s collided (test caught this). Fix: collapse in-memory before iterating — `Dictionary(grouping:by:)` then `.mapValues { $0.reduce(into: TokenBreakdown.zero) { $0 = $0.adding($1.tokens) } }`. Pattern: any UI rendering that needs "X grouped by Y" should pre-aggregate; iterating raw tuples duplicates identifiers and confuses snapshot diffing in UI tests.
- **F2b `.kimiCN` Split via providerID Signal**: Adding a sub-provider to F2b's `Provider` enum doesn't require changing every data source. `OpenCodeExtractor` naturally distinguishes via its raw `providerID` field (`cn` / `kimi-cn` → `.kimiCN`); `KimiCLILegacyExtractor` and `KimiCodeExtractor` stay merged as `.kimi` because their raw data has no "CN" signal (hardcoded `moonshot`). Pattern: when splitting a normalized enum, identify the discrimination source first; if one data source lacks the signal, accept the partial split rather than over-engineering env-var sniffing that may never fire.
- **Surface F2b `initError` in UI Read APIs**: `TokenUsageStore.fetchX` returns `[]` / `TokenBreakdown.zero` silently when `actor.initError` is set. That's correct for graceful degradation but breaks UI consistency — F2a cost section shows "用量数据不可用" while F1/F4 token stats naively render "0 / 0.0k". Fix: caller checks `await store.initError != nil` BEFORE populating the cache. Pattern: when an actor exposes an explicit `initError`, every UI-facing read path that reads from it should early-return or substitute a "(no data)" placeholder when `initError != nil`, mirroring how the F2a cost section already behaves.

### F1/F3/F4 Redesign Patterns (2026-07-09)
- **Materialized History Tables for API Quota State, Not Event Data**: When a UI redesign adds history-style display ("每日记录", "每周记录") of API quota state (5h / 7d usage percent), the cleanest path is a SEPARATE materialized table keyed on `(provider, window, snapshot_ts)` populated by a periodic sample during the existing refresh tick. Don't try to derive history from per-event `token_events` — quota state is qualitatively different (it's the API's percent-of-quota snapshot, not your token consumption). Pattern: `enum XPeriod { case today, week, month }` with one materialized table + one periodic upsert (`INSERT OR IGNORE` for idempotency) + one range query (`since / limit`). The PK on `(provider, window, snapshot_ts)` plus `idx_quota_snapshots_pw_ts (provider, window, snapshot_ts DESC)` gives a stable, append-only history that doesn't compete with `token_events` for write locks.
- **Top-Level Menu Items vs Grouped Submenu is a Navigation Pattern, Not Aesthetics**: When a user asks for "N independent top-level items each with a submenu" instead of "1 grouped submenu with N rows", the difference isn't aesthetic — it changes the navigation pattern. Three independent top-level items means the user sees 今日/本周/本月 totals at the menu ROOT without an extra click. The grouped submenu requires an extra click + visual scanning. The choice matters when the totals are the primary KPI (here: the user wants to know at-a-glance "how much have I spent today/this week/this month" without opening another menu). Pattern: when the top-level number is the answer, expose it as a top-level item; only nest under a submenu when the user needs to drill INTO the number (e.g. per-provider breakdown). 1 separator above the top-level group signals "this is one logical block" to the user.
- **In-Memory Cache for Sync Menu Rebuild**: The `cachedPerPeriodTokens: [TokenPeriod: [ProviderTotal]]` and `cachedQuotaSnapshots: [String: (5h: [...], 7d: [...])]` caches were added so `updateMultiProviderMenu` can stay synchronous. Without them, every menu rebuild would await 2+ actor queries (and `updateMultiProviderMenu` is on the main actor with no async context). Pattern: `refreshXCache()` populates cache + triggers menu rebuild, `updateMultiProviderMenu` reads cache directly. The cache is rebuilt on every 5s tick so freshness is bounded by tick interval, not by menu open events. Same pattern as the existing `refreshMonthlyTotalsCache` — extend the same pattern to any new periodic data source that needs to appear in the menu.
